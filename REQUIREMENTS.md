...
|---|---|---|
| **Office** (was Consultorio) | number, monthly_module_price, area_size, equipment, specialties, active | No image field. |
...
- **H2/Flyway config**: `jdbc:h2:file:./data/consultorios-db` for the real app;
  `jdbc:h2:mem:consultorios-test;DB_CLOSE_DELAY=-1` for tests (the `DB_CLOSE_DELAY=-1` matters —
  without it an in-memory H2 DB can vanish between pooled-connection closes mid-test-run).
  `./data/` gitignored, same convention later for `data/payment-evidence/`.
